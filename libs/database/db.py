...
class database(db_utils):
    ...
    def check_data_for(self, id):
        ...
        for cut in range(max_cut_num):
            ...
            # torch.from_numpy(
            #     seg  * video_item['fps'] #/ feat_stride - feat_offset   # TODO
            # )
            ...
            for anno in temporal_anno_data:
                ...
                # In seconds,
                # print("-- raw label:", label, ", start:", anno["start"], ", end:", anno['end'])
                # print("-- label:", label, ", start:", real_start, ", end:", real_end)
            ...
            self.temporal_label_dict["database"][self.cacheid] = {"duration": duration, "resolution": [256, 512],
    ...
    def check_data_qualify(self, max_cut_num=6):
        ...
        # pool.map(self.check_data_for, self.idlist)
        # pool.close()
        # pool.join()
        ...
        # # 并行计算每个数字的平方
        # results = Parallel(n_jobs=-1)(delayed(self.check_data_for)(num) for num in self.idlist)
        # end_time = time.time()
        ...
        ids = {"train": self.train_ids, "val": self.val_ids, "test": self.test_ids,
               "train_cut": self.train_id_cutwise, "val_cut": self.val_id_cutwise, "test_cut": self.test_id_cutwise}
