...
def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0):
    ...
    for k, v in json_db.items():
        ...
        for event in ants:
            starts += [float(event['segment'][0])]  #  % 10
            stops += [float(event['segment'][1]) ]   # % 10
...
def muses_load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0):
    # load json file
    if json_file.endswith(".npy"):
        json_db = np.load(json_file, allow_pickle=True).item()
    else:
        with open(json_file, "r", encoding="utf8") as f:   # load 
            json_db = json.load(f)
    ...
    for k, v in json_db.items():
        ...
        for event in ants:
            ...
            label_ids = [event['label_id']]
                        #[_label
                        #  for _label in event['label_id']]
            
            #class_names.index(_label)
...
class ANETdetection(object):
    ...
    def wrapper_compute_average_precision(self, preds):
        ...
        print("preds in wrapper:", preds)
        print("gt in wrapper:", self.ground_truth)   # empty GT
    ...
    def evaluate(self, preds, verbose=True):
        ...
        print("final pred in metric:", preds)
...
def compute_average_precision_detection(
    ...
    sort_idx = prediction['score'].values.argsort()[::-1]        # TODO
    prediction = prediction.loc[sort_idx].reset_index(drop=True)
    prediction = prediction.query("`t-end` - `t-start` > 0.2").reset_index(drop=True)
    # prediction_new = prediction.query("score>0.01")


    fp = np.ones((len(tiou_thresholds),   len(prediction)))
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        tiou_arr = segment_iou(this_pred[['t-start', 't-end']].values,
                               this_gt[['t-start', 't-end']].values, post_thr = post_thr)
        ...
        for tidx, tiou_thr in enumerate(tiou_thresholds):
            if tp[tidx, idx] == 1:
                break
            for jdx in tiou_sorted_idx:         # each seg
                if tiou_arr[jdx] < tiou_thr:    # tiou score
                    continue
                tp[tidx, idx] = 1
                fp[tidx, idx] = 0
...
def segment_iou(target_segment, candidate_segments, post_thr=0.0):
    ...
    -------
    tiou : 1d array
        Temporal intersection over union score of the Na's candidate segments.
    """
    target_segment     = np.round(target_segment, 2).clip(0, 10)
    candidate_segments = np.round(candidate_segments, 2).clip(0, 10)

    if post_thr > 0.0:
        if target_segment[0] < post_thr:        target_segment[0] = 0.0
        if target_segment[1] > 10.0 - post_thr: target_segment[1] = 10.0
    ...
    segments_union = segments_union.clip(0)

    # Compute overlap as the ratio of the intersection
    # over union of two segments.
    tIoU = segments_intersection.astype(float) / segments_union.astype(float)
