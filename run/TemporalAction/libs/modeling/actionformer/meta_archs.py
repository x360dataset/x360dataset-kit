...
class PtTransformer(nn.Module):
    ...
    def forward(self, video_list):
        ...
        dict = self.preprocessing(video_list)
        batched_inputs, batched_masks = self.gen_mask(dict)
        ...
        if self.training:
            ...
            # print("segments, labels:", video_list)

            # gt_segments = [x['segments'].to(self.device) for x in video_list if x['segments']!=None]
            # gt_labels = [x['labels'].to(self.device) for x in video_list if x['labels']!=None]
    ...
    def preprocessing(self, video_list, padding_val=0.0):
        ...
        return (batched_inputs, feats_lens, max_len)

    def gen_mask(self, feat_dict):

        (batched_inputs, feats_lens, max_len) = feat_dict
        # generate the mask
    ...
    def label_points_single_video(self, concat_points, gt_segment, gt_label):
        ...
        try:
            ...
            print("failed F.one_hot", gt_label, gt_label.shape, self.num_classes)

        # print("min_len_mask:", min_len_mask.shape)
        try:
            cls_targets = min_len_mask @ gt_label_one_hot
        except:
            print("failed min_len_mask @ gt_label_one_hot", min_len_mask.shape, gt_label_one_hot.shape)
    ...
    def inference_single_video(
        ...
        for cls_i, offsets_i, pts_i, mask_i in zip(
            ...
            # Apply ing to make NMS faster following detectron2
