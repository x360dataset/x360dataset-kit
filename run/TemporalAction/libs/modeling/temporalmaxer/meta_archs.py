...
# Main TemporalMaxerArchitecture Model
...
class TemporalMaxerArchitecture(nn.Module):
    ...
    def label_points_single_video(self, concat_points, gt_segment,
        ...
        for idx, label in enumerate(gt_label):
            ...
            gt_onehot_labels.append(gt_label_one_hot.unsqueeze(0))
            
        gt_label_one_hot = torch.cat(gt_onehot_labels, dim=0)

        cls_targets = assign_mask @ gt_label_one_hot     # failed here
...
class AttentionModel(nn.Module):
    def __init__(self, sequence_length):
        ...
        self.attention = nn.MultiheadAttention(embed_dim=sequence_length, num_heads=1)

    def forward(self, D, A, V):

        attn_output, _ = self.attention(query=D, key=A, value=V)
