...
def concat(f, x):
    ...
    if f == None and x== None:
        print("WARNING: both f and x are None in concat")
        return 
    elif f == None:
        return x
    elif x == None:
...
class Trainer(nn.Module):
    def __init__(self, args, num_classes=1):
        ...
        if args.model == 'r21d':
            ...
            self.model = i3d.I3D(dropout_prob=args.droprate)  # 1024
        ...
        if not self.args.audio_only:
            feature_size += videofeature_size * number_of_videomodality
        ...
        if args.Ms['audio']:
            audio_feature = 128
            feature_size += audio_feature * number_of_videomodality
            ...
            parameters = list(parameters) + list(self.audio_head.parameters())
        ...
        if args.Ms['at']:
            ...
            parameters = list(parameters) + list(self.at_head.parameters())

        print(f'feature_size:{feature_size}')
        embedding_dim = 256
        latent_dim = 512  # 2048..

        self.fusion = TransformerEncoderOneDimension(channel_dim=feature_size,
    ...
    def get_feature(self, args, data_input, tags, device):
        ...
        if args.Ms['at']:
            ...
            audio_map = audio_map.unsqueeze(1)
        ...
        for tag in tags:
            if not self.args.audio_only:
                f = self.model(data_input[tag]['video'])  # outputs  n, 1024, 1, 1, 1
                f = self.video_fc(f)
                # print("f shape:", f.shape)    # ([6, 1024])
                feats_set.append(["f", f])
            ...
            if args.Ms['audio']:
                ...
                # print(f'2: audio_feat.shape:{audio_feat.shape}')

                if args.Ms['at']:
                    audio_feat = audio_feat.view(audio_feat.shape[0] // args.clip_number, args.clip_number, 128)
                    # print("1audio_feat shape:", audio_feat.shape)
                    audio_feat = audio_map * audio_feat
                    # print("2audio_feat shape:", audio_feat.shape)
                    audio_feat = audio_feat.view(audio_feat.shape[0] * audio_feat.shape[1], audio_feat.shape[2])
                    # print("3audio_feat shape:", audio_feat.shape)
                else:
                ...
                audio_feat = audio_feat.view(data_input[tag]['video'].shape[0] // args.clip_number, args.clip_number, 128)
            ...
            if not self.args.audio_only:
                f = f.view(data_input[tag]['video'].shape[0] // args.clip_number, args.clip_number, 1024)  # f.shape[-1])
    ...
    def get_feature_modality_wise(self, args, data_input, tags, device):
        ...
        if not self.args.audio_only:
            if modality == 1:
                ...
                return None, audio_feat
            elif modality == 3:
                return None, audio_feat, at_feat
    ...
    def forward_aux_head(self, feats_set):
        ...
        for feat in feats_set:
            if feat[0] == "f":
                out_set.append(self.video_head(feat[1]))
            elif feat[0] == "a":
