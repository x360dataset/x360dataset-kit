...
class I3D(nn.Module):
    def __init__(self,
        ...
        self.avg_pool = torch.nn.AdaptiveAvgPool3d(1)  #torch.nn.AvgPool3d((2, 7, 7), (1, 1, 1))  # (N,C,D,H,W),


        self.embeddings = nn.Sequential(
            nn.LayerNorm([1024, 1, 1, 1]),
            nn.ReLU(True)
        )
    ...
    def forward(self, inp, features=None, direct=True):
        ...
        if direct:
            return self.embeddings(out).squeeze(3).squeeze(3).mean(2)
        ...
        if features:
            ...
            out = self.conv3d_0c_1x1(out)  # [ 30/22, 1024, 1, 1, 1] -> [ 30/22, 21, 1, 1, 1]
            out = out.squeeze(3)
            out = out.squeeze(3)
