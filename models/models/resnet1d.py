...
class ResNet1D(nn.Module):
    ...
    def __init__(self, block, layers, modality, num_classes=1000, pool='avgpool', zero_init_residual=False,
        ...
        # self.at_head = nn.Linear(self.au_embed, self.n_classes)
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        x = self.layer4(x)
        # out = self.avgpool(x)  # feat
        out = x.mean(2)
        out = self.at_fc(out)
        # out = self.at_head(out)
...
def _resnet(arch, block, layers, modality, progress, **kwargs):
    model = ResNet1D(block, layers, modality, **kwargs)
    return model


def resnet18_1d(modality, progress=True, **kwargs):
