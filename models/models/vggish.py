...
from .vggish_src import waveform_to_examples, wavfile_to_examples
import numpy as np
import torch
...
class VGG(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x shape before emdedding:", x.shape)
...
class VGGish(VGG):
    def __init__(self, train=True, pretrained=True, num_classes=1, dropout_prob=0.1,
                 preprocess=False, postprocess=True, progress=True):
        ...
        if self.postprocess:
            self.pproc = Postprocessor()  # feature
    ...
    def forward(self, x, device=torch.device('cuda'), fs=None):
        if self.preprocess:
            x = self._preprocess(x, device, fs)

        x = VGG.forward(self, x)  # embed 128
