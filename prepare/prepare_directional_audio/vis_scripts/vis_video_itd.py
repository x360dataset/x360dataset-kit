...
def inference(args, pr, net, criterion, data_set, data_loader, device='cuda', video_idx=None):
    ...
    with torch.no_grad():
        for step, batch in tqdm(enumerate(data_loader), total=len(data_loader), desc="Inference"):
            ...
            # ---- save path
            print('img_paths:', img_paths[0])
            save_path = img_paths[0]
            name = save_path.split('/')[-1]
            x360_path = save_path.rstrip(name).rstrip('/frames/')
            
            save_path = os.path.join(x360_path, "front_audio_tracking.npy")
